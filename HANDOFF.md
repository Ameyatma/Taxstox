...
- Backend: `apps/api/Dockerfile`, `apps/api/railway.json` — Railway deployment
- `apps/api/requirements.txt` — pinned production dependencies
- `.env.example` files in both frontend and backend
- Recommended architecture: Vercel (frontend, free) + Railway (backend, ~$5/mo) → taxstox.com
...
- Backend → Railway (point `api.taxstox.com` DNS)
- Set `NEXT_PUBLIC_API_URL=https://api.taxstox.com/api/v1` in Vercel
- Set `TAXSTOX_JWT_SECRET=<random-32-chars>` in Railway
- Configs already created: `vercel.json`, `Dockerfile`, `railway.json`
